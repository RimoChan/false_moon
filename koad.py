...
def 壓(圖,l):
    r,c=圖.shape[:2]
    if r>c:
        return cv2.resize(圖,(c*l//r,l),interpolation=cv2.INTER_AREA)
    else:
        return cv2.resize(圖,(l,r*l//c),interpolation=cv2.INTER_AREA)


def 切分(圖, l, dropout=0):
    r,c=圖.shape[:2]
...
def 遍歷讀圖(路徑, 要名=False, 有限=False, 僅3通道=False):
    while True:
